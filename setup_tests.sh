...
if [ "$el_version" = "6" ]; then
    ...
    ls -l $(pwd)/django-tastypie
    id

    docker run --privileged -d -ti -e "container=docker"  -v /sys/fs/cgroup:/sys/fs/cgroup -v $(pwd):/manager-for-lustre-dependencies:rw  centos:centos${OS_VERSION}   /usr/sbin/init
